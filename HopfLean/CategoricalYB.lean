...
* ≫ : composition of morphisms in a category (the reverse of usual composition,
  i.e. g ≫ f := f ∘ g)
-/
...
theorem cat_yang_baxter {C : Type u} [Category.{v, u}    C] [MonoidalCategory C]
  ...
  intros X Y Z
  /- this key follows immediately from the reverse hexagon axiom, but associativity
  and post/precomposition of morphisms complicate things -/
  have key : ∀ X Y Z : C, (α_ X Y Z).hom ≫ (((𝟙 X) ⊗ (β_ Y Z).hom) ≫
    ...
    /- simplify using associativity of morphism composition, the definition of
  ...
  have aux : (α_ X Y Z).hom ≫ (𝟙 X ⊗ (β_ Y Z).hom) ≫ (α_ X Z Y).inv ≫
    ...
    simp only [assoc]
  /- two applications of the reverse hexagon axiom and naturality of the
  braiding closes the goal -/
